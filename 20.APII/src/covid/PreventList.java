...
//병원목록화면
public class PreventList {
	private PublicAPI api;	//필드의 초기화: 생성자, setter 메소드
	
	public PreventList(PublicAPI api) {
		this.api = api;
	}
	public void display() {
		//API를 요청한다
		//https://api.odcloud.kr/api/apnmOrg/v1/list?page=1&perPage=10&serviceKey=data-portal-test-key&returnType=JSON
		//https://api.odcloud.kr/api/15077586/v1/centers?page=1&perPage=10&serviceKey=data-portal-test-key&returnType=JOSN
		StringBuffer url = new StringBuffer("https://api.odcloud.kr/api/apnmOrg/v1//centers");
...
//		viewConsole(url);
	...
	private void ViewHtml(StringBuffer url) {
		...
		try {
			...
			out.print("<html>");
			out.print("<body>");
			out.print("<h3>코로나 예방접종센터 위치정보</h3>");
			out.print("<table border='1' style='width:1000px'>");
			out.print("<tr><th style='width:320px>센터명</th>");
	...
	public void viewConsole(StringBuffer url) {
		System.out.println("센터명 \t 전화번호 \t 주소");
		JSONArray array = api.covidAPItoJSON(url);
		for(int idx = 0; idx<array.length(); idx++) {
			JSONObject hospital = array.getJSONObject(idx);
